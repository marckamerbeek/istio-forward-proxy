...
- [ ] **F1 ([#25](https://github.com/marckamerbeek/istio-forward-proxy/issues/25)) — Audit identity is forgeable by the client.** In ambient mode `r.TLS` is
...
- [ ] **F2 ([#26](https://github.com/marckamerbeek/istio-forward-proxy/issues/26)) — Chunked request bodies vanish, and their bytes are fed to the next hop as
...
- [ ] **F9 ([#27](https://github.com/marckamerbeek/istio-forward-proxy/issues/27)) — `--idle-timeout` is an absolute deadline from connection start, not an
...
- [ ] **F10 ([#28](https://github.com/marckamerbeek/istio-forward-proxy/issues/28)) — CONNECT tunnels have no timeout at all.** `handleConnect`/`tunnel()` set
...
- [ ] **F11 ([#29](https://github.com/marckamerbeek/istio-forward-proxy/issues/29)) — No upstream connection pooling.** No `http.Transport`/`http.Client` reuse
...
- [ ] **F3 ([#30](https://github.com/marckamerbeek/istio-forward-proxy/issues/30)) — A failing upstream gives the client nothing, and the audit log says 503.**
...
- [ ] **F4 ([#31](https://github.com/marckamerbeek/istio-forward-proxy/issues/31)) — `bytes_out` is always 0 in the per-request audit record**, including on
...
- [ ] **F5 ([#32](https://github.com/marckamerbeek/istio-forward-proxy/issues/32)) — The audit record logs the raw client string, not the matched/normalized
      ...
      client-controlled.
- [ ] **F13 ([#33](https://github.com/marckamerbeek/istio-forward-proxy/issues/33)) — The `Expect: 100-continue` expectation is answered locally instead of
...
- [ ] **F6 ([#34](https://github.com/marckamerbeek/istio-forward-proxy/issues/34)) — `:9090/metrics` is unauthenticated and reachable cluster-wide.** No
...
- [ ] **F7 ([#35](https://github.com/marckamerbeek/istio-forward-proxy/issues/35)) — Nothing ties `mtls.enabled` to the upstream's actual scheme.** A plain-port
...
- [ ] **F8 ([#36](https://github.com/marckamerbeek/istio-forward-proxy/issues/36)) — Default `RollingUpdate` strategy stalls with `replicaCount: 1`** on a node
      ...
      `strategy: Recreate`.
- [ ] **F12 ([#37](https://github.com/marckamerbeek/istio-forward-proxy/issues/37)) — `Proxy-Connection` (hop-by-hop) is relayed to the parent.** Correctly
